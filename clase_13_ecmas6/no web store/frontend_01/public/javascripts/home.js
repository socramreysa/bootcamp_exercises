...
/*$("#buttonNewProduct").click(function(){
  ...
  $('#newProduct').show()
    const div = document.getElementById('selectCategorias');
    const url = 'http://172.50.1.122:3000/categorias';
    fetch(url)
    .then((resp) => resp.json())
      .then(function(data) {
        let categorias = data;
...
$("#crearNewCategoria").click(function(){
  ...
  var data = {
    ...
    $.ajax({
      method: 'POST',
      url: 'http://172.50.1.122:3000/newCategoria',
      data: data,
      success: function(data){
          const div = document.getElementById('selectCategorias');
          const url = 'http://172.50.1.122:3000/categorias';
...
$(".buttonOneProduct").click(function(){
   HideAll()
    fetch('http://172.50.1.122:3000/'+this.value)
    .then((resp) => resp.json())
      .then(function(data) {
...
////////////////////////////////////////////SHOW --> ALL PRODUCTS
 ...
 $("#buttonProducts").click(function(){
   HideAll()
    fetch('http://172.50.1.122:3000/products')
    .then((resp) => resp.json())
      .then(function(data) {
        return function(data) {
         $('#container').append("<div class='row'></div>")
         for (var i = 0; i < data.length; i++) {
            $('.row').append("<div id='"+i+"' class='col-sm-6 col-md-4'></div>")
            $('#'+i).append("<div id='b"+i+"' class='thumbnail'></div>")
            $('#b'+i).append('<img class="imagenes" src='+data[i].imgUrl+'>')
            $('#b'+i).append("<div id='c"+i+"'class='caption'>")
            $('#c'+i).append("<h3>"+data[i].nombre+"</h3>")
            $('#c'+i).append("<p>$"+data[i].precio+"</p>")
            $('#c'+i).append('<p><a name='+data[i].id+' href="#" class="btn btn-primary buttonOneProduct" role="button">Ver más</a></p>')
         }
